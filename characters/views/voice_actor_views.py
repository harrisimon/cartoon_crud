...
class VoiceActorView(APIView):
    """View class for voice_actors / viewing all and creating"""
...
class VoiceActorDetailView(APIView):
        """View class for voice_actors/:pk for viewing a single voice_actor, updating a single voice_actor, or removing a single voice_actor"""
        serializer_class = VoiceActorSerializer
        def get(self, request, pk):
            voice_actor = get_object_or_404(VoiceActor, pk=pk)
            serializer = VoiceActorSerializer(voice_actor)
            return Response({'voice_actor':serializer.data})
